...
<div style="width:800px; margin-left: 200px; margin-top: 60px; padding-bottom: 50px; padding-left: 20px;padding-right: 10px; width: 900px; background:white; border:2px solid gray;" class="newA">
	...
	<figcaption>The Hoba Meteorite in Namibia</figcaption>
     ...
     <h3>Passwords Suck Like Getting Hit By This Thing!</h3>
...
<p>
Next up was what would the credential store protect? How about private keys in public/private key pairs? A private key can sign a request for login/joining a web site and the only thing the server host has to do is remember the public key that is bound to an identity, such as your user name or email. The good thing about public keys is... that they are public! If you have a server breach the public keys bound to the account tell attackers just about nothing. So my part of the HOBA experiemental RFC<a href="https://tools.ietf.org/html/rfc7486">(RFC 7486)</a> set about to implement a javascript based public key signer for login/joining, and a credential store. My javascript version was very much a prototype, and even if I did rationalize a little that it might not be so horrible, I knew in my heart that it was. Using Math.random() was enough to disqualify it. So the point was not that the pig sang well, it was that the pig sang at all.
</p>
<p>
So why pick this back up after 8 years? Two things: WebAuthn and WebCrypto. I was very happy to hear when <a href="https://www.w3.org/TR/webauthn/">WebAuthn</a> became a w3c standard. It was quite surprising as it sounded very much like what I wanted to do, but was limited by the lack of browser primitives. Unfortunately as I tried to implement a version using it, the reality was very different. Webauthn is extremely joined at the hip to hardware based crypto frobs to sign credentials. What I wanted to do is just get rid of passwords on the wire, not replace them entirely. That is either not possible at all currently for Chrome, and barely possible on Firefox using a krufty about:config flag. The part really bothered me was that while I implicitly understood what WebAuthn was trying to do, it was <i>really</i> hard to understand. And if I'm having trouble slogging through it, web folks who don't have much in the way of crypto experience are probably hopeless.
</p>
<p>
Fortunately something else happened in the mean time: <a href="https://www.w3.org/TR/WebCryptoAPI/">WebCrypto</a>. WebCrypto gives all of the tools that I was lacking with my javascript crypto libraries. It's pretty safe to assume that they are just putting on a browser API veneer to existing crypto libraries used by browsers. Given WebAuthn's complexity and WebCrypto's availability, it seemed reasonable to revisit HOBA again by implementing a new WebCrypto based version. So this site was born.
</p>
<p>
You can visit the my original blog post about what would become HOBA <a href="https://rip-van-webble.blogspot.com/2012/06/using-asymmetric-keys-for-web-joinlogin.html">here</a>. My post implentation experience <a href="https://rip-van-webble.blogspot.com/2012/07/asymmetric-keying-after-implementation.html">here</a> and for giggles my attempt to rationalize some stuff <a href="https://rip-van-webble.blogspot.com/2012/06/localstorage-secuity.html">here</a>.
</p>
<p>
Best of all there is a <a href="https://github.com/shirikodama/hoba-bis" target="_blank">Github Project</a> you can clone to puruse the code. I tried to chop this down to be just demonstrate the general problem and my solutions for various (re)enrollment scenarios.
</p>
<p>
